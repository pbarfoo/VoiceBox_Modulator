...
function FilePick({
  label, sublabel, file, onPick,
}: { label: string; sublabel?: string; file: File | null; onPick: (f: File | null) => void }) {
      ...
      {!file && (
        <div className="flex justify-center mb-2">
          <svg width="20" height="20" viewBox="0 0 20 20" fill="none" style={{ color: MUTED }}>
            <path d="M10 3.5a.5.5 0 0 1 .5.5v6.793l2.146-2.147a.5.5 0 0 1 .708.708l-3 3a.5.5 0 0 1-.708 0l-3-3a.5.5 1 1 1 .708-.708L9.5 10.793V4a.5.5 0 0 1 .5-.5Z" fill="currentColor"/>
...
const STAGES = ["Starting","Loading audio","Extracting speech","Spectrograms",
  "Target voice","Aligning","Diffusion","Synthesising","Saving","Complete"];
    ...
    <div className="space-y-3">
      ...
      <div className="h-[3px] rounded-full overflow-hidden" style={{ background: "rgba(255,255,255,0.07)" }}>
        <div className="h-full rounded-full transition-all duration-500" style={{
          width: `${pct}%`,
          background: isFailed ? "#f87171" : isDone
      ...
      <div className="flex gap-1">
        {STAGES.map((s, i) => {
          ...
          return (
            <div key={s} title={s} className="flex-1 h-[3px] rounded-full transition-all duration-300"
              style={{
                background: isFailed && i >= current ? "rgba(248,113,113,0.2)"
                  : done ? "rgba(124,77,255,0.7)" : active ? "#7c4dff" : "rgba(255,255,255,0.07)",
  ...
  return (
    <div className="p-8 max-w-[600px] mx-auto space-y-6">
      ...
      <div>
        ...
        <FilePick label="Source recording" sublabel="The audio you want to re-voice"
          file={source} onPick={(f) => { setSource(f); reset(); }} />
      </div>
      ...
      <div>
        ...
        {profiles.data && profiles.data.length > 0 && (
          <select value={profileId} onChange={(e) => { setProfileId(e.target.value); reset(); }}
        ...
        {!profileId && (
          <FilePick label="Target sample" sublabel="1–30s clean recording of the target speaker"
            file={target} onPick={(f) => { setTarget(f); reset(); }} />
        )}
      </div>
      ...
      {job && (job.status === "queued" || job.status === "running") && (
        <div className="rounded-xl px-5 py-4" style={{ background: SURFACE, border: `1px solid ${BORDER}` }}>
          <div className="text-[11px] uppercase tracking-widest mb-3" style={{ color: MUTED }}>Processing</div>
          <JobProgress job={job} />
        </div>
      )}
      ...
      {job?.status === "failed" && (
        <div className="rounded-xl px-5 py-4 space-y-3"
          ...
          <div className="text-[12px] font-medium" style={{ color: "#f87171" }}>Conversion failed</div>
          <JobProgress job={job} />
        </div>
      )}
      ...
      {job?.status === "done" && resultUrl && (
        <div className="rounded-xl px-5 py-4 space-y-4" style={{ background: SURFACE, border: `1px solid ${BORDER}` }}>
          <div className="flex items-center gap-2">
            <span className="w-1.5 h-1.5 rounded-full"
              style={{ background: "#34d399", boxShadow: "0 0 6px #34d399" }} />
            <span className="text-[12px]" style={{ color: "rgba(52,211,153,0.75)" }}>Conversion complete</span>
          </div>
          <JobProgress job={job} />
          <div style={{ height: 1, background: "rgba(255,255,255,0.05)" }} />
          ...
          <a href={resultUrl} download
            ...
            <svg width="12" height="12" viewBox="0 0 12 12" fill="none">
              <path d="M6 1v7M3 5.5l3 3 3-3M1 10h10" stroke="currentColor" strokeWidth="1.4"
